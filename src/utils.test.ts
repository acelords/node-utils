...
test("insertIntoArray - can insert another array inside an array at a certain index", () => {
  expect(insertIntoArray([1, 2, 3, 4], 2, [8, 9]).join(",")).toBe(
    [1, 2, 8, 9, 3, 4].join(",")
  );
  expect(insertIntoArray([1, 2], 5, [8, 9]).join(",")).toBe(
    [1, 2, 8, 9].join(",")
  );
});
...
test("camelCase - converts a string to camelCase", () => {
  ...
  expect(camelCase("abcd efgh")).toBe("abcdEfgh");
  expect(camelCase("abcdEfgh")).toBe("abcdefgh");
...
test("camelCaseToSentenceCase - converts camelCase string to Sentence case", () => {
  ...
  expect(camelCaseToSentenceCase("myStringIsDope", true)).toBe(
    "My String Is Dope"
  );
...
test("snakeCaseToSentenceCase - converts snake_case string to Sentence case", () => {
  ...
  expect(snakeCaseToSentenceCase("my_string_is_dope")).toBe(
    "My string is dope"
  );

  expect(snakeCaseToSentenceCase("abcd", true)).toBe("Abcd");
  expect(snakeCaseToSentenceCase("my_string", true)).toBe("My String");
  expect(snakeCaseToSentenceCase("my_string_is_dope", true)).toBe(
    "My String Is Dope"
  );
...
test("kebabCaseToSentenceCase - converts kebab-case string to Sentence case", () => {
  ...
  expect(kebabCaseToSentenceCase("my-string-is-dope")).toBe(
    "My string is dope"
  );
  expect(kebabCaseToSentenceCase("My string is dope")).toBe(
    "My string is dope"
  ...
  expect(kebabCaseToSentenceCase("my-string-is-dope", true)).toBe(
    "My String Is Dope"
  );
  expect(kebabCaseToSentenceCase("My String Is Dope", true)).toBe(
    "My  String  Is  Dope"
...
test("birthdayFromNow - can get days to next birthday", () => {
  ...
  expect(birthdayFromNow(dayjs().add(4, "days").toDate())).toBeLessThanOrEqual(
    4
  );
